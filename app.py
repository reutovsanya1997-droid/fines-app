...
SHEET_ID = "1ick_Wz5RSDQ5URkOjeSb2fwra6sz61xkKjbB-y5aYrY"
...
def get_fines_sheet():
    return get_gsheet_client().open_by_key(SHEET_ID).worksheet("fines")

def get_users_sheet():
    return get_gsheet_client().open_by_key(SHEET_ID).worksheet("users")
...
def load_users():
    ...
    for row in data[1:]:
        if row and row[0]:
            users.append({'badge': row[0], 'phone': row[1], 'name': row[2] if len(row) > 2 else f'Сотрудник {row[0]}'})
    return users

def save_user(badge, phone, name):
    get_users_sheet().append_row([badge, phone, name])
...
def save_fine(fine):
    get_fines_sheet().append_row([fine['id'], fine['badge'], fine['fine_date'], fine['description'], fine['comment'], fine['admin_answer'], fine['created_at'], fine['status']])
